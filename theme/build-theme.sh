...
verify_files() {
    ...
    local preview_file
    preview_file=$(node -e "const fs=require('fs'); const m=JSON.parse(fs.readFileSync('komari-theme.json','utf8')); console.log(m.preview || 'preview.png');")

    if [ ! -f "$preview_file" ]; then
        print_error "$preview_file not found"
        files_missing=true
    fi
...
create_package() {
    ...
    find theme-package/dist -name "*.zip" -type f -delete
    
    # Create zip file with version and commit hash
    THEME_SHORT=$(node -e "const fs=require('fs'); const m=JSON.parse(fs.readFileSync('komari-theme.json','utf8')); if(!m.short) process.exit(1); console.log(m.short);")
    ZIP_NAME="${THEME_SHORT}-v${VERSION_DATE}-${COMMIT_HASH}.zip"
